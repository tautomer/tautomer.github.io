...
for f in files:
    if f[f.rfind("-") + 1 : f.rfind(".")] != "thumbnail":
        ...
        #     basename = tmp
        # else:
        #     basename = "-".join(tmp_split[:-1])
        # print(basename, ext)
        # newf = basename + "-%sx%s." % imagesize.get(join(path, f)) + ext
        newf = (
            f[: f.rfind("-")]
            + "-%sx%s" % imagesize.get(join(path, f))
            + f[f.rfind(".") :]
        )
...
for pic in new_gallery:
    ...
    if not found:
        ...
        old_gallery.append(
            {
                "filename": pic,
                "sizes": new_gallery[pic],
                "thumbnail": thumbs[pic],
                "original": originals[pic],
            }
        )
